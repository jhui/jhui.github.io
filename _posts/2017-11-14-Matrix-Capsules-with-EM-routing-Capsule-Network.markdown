...
In the original paper, "$$-b_j$$" is explained as the cost of describing the mean and variance of capsule j. The detail explanation involves basic understanding of energy based models like the Restricted Boltzmann Machines. However, let's say "benefit" is the opposite of the "cost". $$b_j$$ represents the benefit of using the parent capsule to represent datapoints. If such benefit is higher than the $$cost$$ caused by the discrepancy in the votes, we turn on the parent capsules. $$b_j$$ is learned discriminatively using a cost function and the backpropagation. 

$$r_{ij}$$, $$\mu$$, $$\sigma$$ and $$a_j$$ are computed iteratively using EM routing discussed in the next section. λ in the equation above is an inverse temperature $$ \frac{1}{temperature}$$ parameter which increases after each EM routing iteration. As $$a_j$$ and $$r_{ij}$$ are getting more accurate, we can drop the temperature (larger λ) to make it more likely to activate the parent capsule if $$b_j > \sum_h cost^h_j)$$. In our implementation, λ is first initialized to 1 at the beginning of the iterations and then increment by 1 after each routing iteration. The paper does not specific the details and we would suggest experimenting different schemes in your implementation.
