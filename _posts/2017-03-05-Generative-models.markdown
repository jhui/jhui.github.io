...
Here, we pass the first and the second image to the CNN respectively. We extract the corresponding features of those image at layers away from the input. We subtract the difference and use it as the gradient for backpropagation. 
```python
...
We compute the lost function for the generator by using the logits of generated images from the discriminator. Hence, we train the $$W$$ in the generator to create more realistic images.
```python
...
We say we can sample data from this gaussian distribution with mean $$ \mu $$ and standard deviation $$ \sigma $$ with the following notation.
...
In this example, mean: $$ \mu=0 $$, standard deviation: $$ \sigma=0.1$$:
...
which covariance matrix $$ \sum $$ is:
...
$$ E_{21} $$ measures the co-relationship between variables $$x_2$$ and $$x_1$$. Since weight increase with height, we realize the value is positive. If they are independent of each other, it should be 0 like:
...
The CNN encoder does not generate the latent variables directly. Instead it generates a gaussian distribution function for each input image. If we use 20-D for our latent variables, the CNN will generate 20 $$ \mu $$ and 20 $$ \sigma $$ for each image. Then the latent variables are generated by sampling the gaussian distribution.
...
To achieve this, we want the gaussian distribution model generated by the encoder to be as close to a normal gaussian distribution function. We penalize the cost function if the gaussian function is deviate from a normal distribution.
...
In a normal gaussian distribution, the covariance $$ E_{ij} $$ is 0 for $$ i \neq j $$. That is the latent variables are independent of each other. If the distribution is normalize, the distance between different $$z$$ will be a good measure of its similarity. In combination of sampling the latent variable from a gaussian distribution, we encourage the network to have similar value of $$z$$ for similar images.
...
By definition of the KL divergence which measures the difference of 2 distributions: 
...
$$ q_\lambda (z∣x) $$ below is the model from our deep network and we want it to match the true distribution $$ p(z∣x) $$. We want to make sure the distribution approximate from the deep network has little divergence from the true distribution. So we want to find $$ \lambda $$ with the smallest KL divergence.
...
To minimize the KL-divergence. we want to find $$ \lambda $$ to maximize $$ ELBO(λ)$$. (Since $$ log (p(x)) $$ is not dependent on $$ \lambda $$, the term $$\log (p(x))$$ is not important to find the minimum.)
...
#### Result
Here is the digits created by a VAE.
<div class="imgcap">
