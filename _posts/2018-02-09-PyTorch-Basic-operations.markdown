...
By selecting different configuration options, [the tool in the PyTorch site]((http://pytorch.org/#pip-install-pytorch)) shows you the required wheel and the latest version for your host platform. For example, on a Mac platform, the command for pip3 generated by the tool is:
...
Run the following application (basic.py) and you should see a random 2x3 **Tensor** is printed out.  Tensor is a data structure representing multi-dimensional array. It is similar to a NumPy ndarrays.
...
This printout represents the Tensor type and its dimension.
```
[torch.FloatTensor of size 2x3]
```
...
x = torch.Tensor(2, 3)  # An un-initialized Tensor object. x holds garbage
...
All operators end with "_" is in place operations:
...
Operators provide a "out" parameter to store results

```python
result = torch.Tensor(2, 3)
torch.add(x, y, out=result)   # Same as result = torch.add(x, y)
```
